...
export default function ShoppingBag() {
  ...
  const { bagItems, countTotalPrice } = useBagItemsContext();
  ...
  const displayItems = bagItems.map((item: Product) => (
    <li key={item.id} className={styles.listItemContainer}>
      <Link
        to={`/product/${item.id}`}
        state={item}
        className={styles.bagElContainer}
      >
  ...
  return (
    <div>
      ...
      >
        <img
          src="../../src/assets/images/red-bag.svg"
      ...
      {displayItems.length > 0
        ? isDropdownActive && (
            <div ref={dropdownRef} className={styles.dropdown}>
              <ul>{displayItems}</ul>
              <p className={styles.total}>Total: ${countTotalPrice()}</p>
              <Link
